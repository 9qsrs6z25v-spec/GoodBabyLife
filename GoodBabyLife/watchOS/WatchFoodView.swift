...
struct WatchFoodView: View {
    ...
    var body: some View {
        ScrollView {
            VStack(spacing: 12) {
                Text("食物種類").font(.caption2).foregroundStyle(.secondary)
                    ...
                    ForEach(FoodType.allCases) { type in
                        Button { selectedType = type } label: {
                            VStack(spacing: 1) {
                                ...
                                Text(type.label).font(.system(size: 7))
                            }
                ...
                VStack(spacing: 4) {
                    Text("份量").font(.caption2).foregroundStyle(.secondary)
                    HStack {
                        Button { if amount > 1 { amount -= 1 } } label: {
                            Text("−").font(.headline).frame(width: 36, height: 36).background(Color.babyMintLight).clipShape(Circle())
                        }.buttonStyle(.plain)
                        Text("\(amount) 匙").font(.title3).fontWeight(.bold).frame(maxWidth: .infinity)
                        Button { if amount < 50 { amount += 1 } } label: {
                            Text("+").font(.headline).frame(width: 36, height: 36).background(Color.babyMintLight).clipShape(Circle())
                        }.buttonStyle(.plain)
                    }
                }

                Text("寶寶反應").font(.caption2).foregroundStyle(.secondary)
                HStack(spacing: 4) {
                    ForEach(BabyReaction.allCases) { reaction in
                        Button { selectedReaction = reaction } label: {
                ...
                Button {
                    let record = FoodRecord(date: Date(), type: selectedType, name: selectedType.label, amountSpoons: amount, reaction: selectedReaction)
                ...
                .buttonStyle(.borderedProminent).tint(Color.babyMint)
