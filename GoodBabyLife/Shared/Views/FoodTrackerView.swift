...
struct FoodTrackerView: View {
    ...
    private var formSection: some View {
        VStack(alignment: .leading, spacing: 20) {
            ...
            Button(action: saveFoodRecord) {
                ...
                .padding(.vertical, 14)
                .background(BabyGradient.foodButton)
    ...
    private func foodTypeButton(_ type: FoodType) -> some View {
        Button {
            ...
            .overlay(
                RoundedRectangle(cornerRadius: 10)
                    .stroke(selectedType == type ? Color.babyMint : Color.babyMintLight, lineWidth: 2)
            )
        }
        .buttonStyle(.plain)
    ...
    private func reactionButton(_ reaction: BabyReaction) -> some View {
        Button {
            withAnimation(.spring(duration: 0.3)) {
            ...
            .overlay(
                RoundedRectangle(cornerRadius: 10)
                    .stroke(selectedReaction == reaction ? Color.babyPinkDark : Color.babyLavender, lineWidth: 2)
            )
        }
        .buttonStyle(.plain)
        ...
        HStack {
            Button {
                ...
                Text("−")
                    ...
                    .foregroundStyle(Color.babyMint)
                    .clipShape(RoundedRectangle(cornerRadius: 10))
            }
            .buttonStyle(.plain)
    ...
    private var historySection: some View {
        VStack(alignment: .leading, spacing: 12) {
            ...
            if store.todayFoodRecords.isEmpty {
                VStack(spacing: 12) {
                    Text("🥣")
                        .font(.system(size: 48))
                        .opacity(0.5)
                ...
                ForEach(store.todayFoodRecords) { record in
                    foodHistoryRow(record)
                }
            }
        }
        .babyCard()
    ...
    private func foodHistoryRow(_ record: FoodRecord) -> some View {
        HStack(spacing: 12) {
            Text(record.type.emoji)
                .font(.system(size: 28))

            VStack(alignment: .leading, spacing: 2) {
                Text("\(record.displayName) \(record.amountSpoons)湯匙")
                    .font(.subheadline)
                    .fontWeight(.semibold)
                Text("\(record.reaction.emoji) \(record.reaction.label)\(record.note.isEmpty ? "" : " · \(record.note)")")
                    .font(.caption)
                    .foregroundStyle(Color.babyTextLight)
            }
